...
"""使用本地 faster-whisper 生成视频字幕（默认引擎，GPU 加速，无 API Key）。

特点：
- 本地模型（FASTER_WHISPER_MODEL_PATH，默认 large-v3），无需联网/密钥
- word_timestamps 字/词级毫秒时间戳 → MAW 工程 items
- --device/--compute-type 控制 CUDA/CPU 与精度；VAD 默认开
- 无说话人分离（faster-whisper 不含 diarization，--speaker 会被忽略并警告）

输出为通用的 UTF-8 JSON 工程格式（默认 .mosp），可直接交给 edit.py 编辑。
配置读取 .env 文件（FASTER_WHISPER_* 键）。
"""
...
from maw.faster_whisper import build_segments, load_config, transcribe
from maw.project import repair_segment_durations, validate_project
from waveform import embed_waveform


def _language_code(raw: str | None) -> str | None:
    """'zh' / 'Chinese' / 'ja' → faster-whisper ISO 码（单语言）。"""
    if not raw:
        return None
    key = raw.strip().lower()
    if key in LANGUAGE_MAP:
        return LANGUAGE_MAP[key]
    # 已经是 ISO 码（zh/en/ja...）则直接用
    return key if len(key) <= 3 else None


def main():
    parser = argparse.ArgumentParser(
        description="使用本地 faster-whisper 生成视频字幕（默认引擎，GPU 加速）",
    )
    ...
    parser.add_argument(
        ...
        help="语言提示（如 zh / en / ja / Chinese；默认自动识别）",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="开启说话人分离（本地引擎不支持，会警告并忽略）",
    )
    parser.add_argument(
        "--speaker-colors", action="store_true",
        help="同 --speaker（本地引擎不支持）",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model", default=None,
        help="覆盖模型（尺寸名或本地模型目录；默认读 .env 的 FASTER_WHISPER_MODEL）",
    )
    parser.add_argument(
        "--device", default=None,
        help="设备（auto/cuda/cpu；默认读 .env 的 FASTER_WHISPER_DEVICE）",
    )
    parser.add_argument(
        "--compute-type", default=None,
        help="计算精度（float16/int8；GPU 默认 float16，CPU 默认 int8）",
    )
    parser.add_argument(
        "--no-vad", action="store_true",
        help="关闭 VAD（默认开，silero 过滤静音）",
    )
    parser.add_argument(
        "--debug", action="store_true",
        help="输出 items 解析结果用于调试",
    )
    ...
    if args.no_vad:
        config["vad"] = False
    print(f"[准备] 已载入 faster-whisper 配置（模型: {args.model or config['model']}，"
          f"device: {config['device']}，compute: {config['compute_type'] or 'auto'}，"
          f"VAD: {'on' if config['vad'] else 'off'}）")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("[faster-whisper] 本地媒体准备完成，开始转写...")
        t0 = time.perf_counter()
        result = transcribe(
            audio_path, config,
            language=_language_code(args.language),
            model_size=args.model,
            enable_speaker=bool(args.speaker or args.speaker_colors),
        )
        ...
        print(f"[解析] 本地转写已返回，包含 {len(result.get('items', []))} 个时间戳项。")
    ...
    if not args.output:
        ...
        output_path = output_path.with_name(
            f"{ts_prefix}{output_path.stem}.faster-whisper.{speed_tag}.srt"
        )
    ...
    if args.json_out:
        ...
        json_data = {
            ...
            "model": f"faster-whisper-{args.model or config['model']}",
            "segments": [
                {
                    "start": seg["start"],
                    "end": seg["end"],
