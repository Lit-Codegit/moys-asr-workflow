...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.transcribe_faster_whisper:
        return _run_internal_transcribe_faster_whisper(rest)
    if args.serve:
...
def _run_internal_transcribe_faster_whisper(argv: Sequence[str]) -> int:
    import generate_subtitle_faster_whisper_api

    old_argv = sys.argv[:]
    try:
        sys.argv = ["generate_subtitle_faster_whisper_api.py", *argv]
        result = generate_subtitle_faster_whisper_api.main()
    finally:
        sys.argv = old_argv
    return 0 if result is None else int(result)
