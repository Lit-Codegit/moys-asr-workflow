...
class GuiWebBridgeTests(unittest.TestCase):
    ...
    def test_get_config_returns_registry_and_masked_key_when_env_exists(self) -> None:
        ...
        # 默认引擎 faster-whisper 无 API Key；DASHSCOPE key/region 属于 qwen（第二供应商，保留在 .env）
        self.assertEqual(config["apiKey"], "")
        self.assertEqual(config["maskedApiKey"], "")
        self.assertEqual(config["region"], "singapore")
        self.assertEqual(config["guiLang"], "en")
        self.assertEqual(config["providerId"], "faster-whisper")
        self.assertEqual(config["modelId"], "large-v3")
        self.assertIsNone(config["lastModel"])
        self.assertIsNone(config["lastLanguage"])
        self.assertEqual(config["stickerDir"], "")
        self.assertEqual(config["providers"][0]["keyUrl"], "")
        self.assertEqual(len(config["providers"][0]["commonLanguages"]), 8)
        self.assertEqual(len(config["providers"][1]["commonLanguages"]), 10)
        self.assertEqual(config["models"][0]["id"], "large-v3")
        self.assertEqual(config["models"][1]["id"], "small")
        # large-v3 本地模型：无 speaker/context/hotwords/vocabulary 能力
        self.assertFalse(config["models"][0]["supportsSpeaker"])
        self.assertFalse(config["models"][0]["supportsContext"])
        self.assertFalse(config["models"][0]["supportsHotwords"])
        self.assertFalse(config["models"][0]["supportsVocabulary"])
        self.assertEqual(config["models"][0]["languages"][0]["id"], "zh")
