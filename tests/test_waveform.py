...
class EditorAssetTests(unittest.TestCase):
    ...
    def test_blank_editor_inlines_modular_assets(self) -> None:
        ...
        # 决策 43：旧「字幕预览设置」面板（全局字号/字体）被 ASS 样式体系取代
        self.assertNotIn('id="subtitle-font-size"', page)
        self.assertNotIn('id="subtitle-preview-settings-panel"', page)
        self.assertIn('id="style-manager-btn"', page)
        self.assertIn('id="style-manager-modal"', page)
        self.assertIn('id="style-panel-modal"', page)
        self.assertIn('id="style-conflict-modal"', page)
        self.assertIn('assOverlayCss', page)
        self.assertIn('resolveSegmentStyleName', page)
        self.assertIn('normalizeAssStyle', page)
