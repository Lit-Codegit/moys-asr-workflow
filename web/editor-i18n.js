(function initMaweI18n(global) {
  ...
  const EN_ATTR = {
    ...
    '样式管理器': 'Style Manager',
    '工程样式': 'Project styles',
    '本机目录（Aegisub ~/.aegisub/catalog）': 'Local catalog (Aegisub ~/.aegisub/catalog)',
    '新建': 'New',
    '编辑': 'Edit',
    '重命名': 'Rename',
    '删除': 'Delete',
    '设为默认': 'Set as default',
    '导入所选目录样式': 'Import selected catalog styles',
    '导入 .sty/.ass 文件…': 'Import .sty/.ass file…',
    '颜色映射（任务级：颜色 → 样式）': 'Color mapping (per project: color → style)',
    '关闭': 'Close',
    '字幕样式': 'Subtitle style',
    '恢复默认': 'Reset to defaults',
    '应用': 'Apply',
    '样式名冲突': 'Style name conflict',
    '跳过': 'Skip',
    '改名导入': 'Import renamed',
    '覆盖': 'Overwrite',
    '新名称': 'New name',
    '样式': 'Style',
    '继承颜色绑定': 'Inherit color binding',
    '字体': 'Font',
    '字号': 'Font size',
    '粗体': 'Bold',
    '斜体': 'Italic',
    ...
    '旋转°': 'Angle',
    '对齐': 'Alignment',
    '位置（PlayRes）': 'Position (PlayRes)',
    '清除位置': 'Clear position',
    '淡入/淡出 ms': 'Fade in/out ms',
    '无绑定': 'None',
    '第': 'Segment',
    '条': '',
    '编辑样式': 'Edit style',
    '改动对所有引用该样式的字幕生效': 'Changes apply to every subtitle using this style',
    '修改字幕样式': 'Edit subtitle style',
    '修改样式': 'Edit style',
    '重置字幕样式': 'Reset subtitle style',
    '重置样式': 'Reset style',
    '已恢复该段的样式默认值': 'Restored default style for this segment',
    '已恢复样式默认值': 'Restored style defaults',
    '拖动字幕位置': 'Drag subtitle position',
    '导入样式': 'Import styles',
    '新建样式': 'New style',
    '重命名样式': 'Rename style',
    '删除样式': 'Delete style',
    '设为默认样式': 'Set as default style',
    '颜色样式映射': 'Color-style mapping',
    '仅服务器编辑器支持读取本机 Aegisub 目录': 'Only the server editor can read the local Aegisub catalog',
    '目录不存在': 'Directory not found',
    '读取失败': 'Failed to load',
    '没有解析到任何样式': 'No styles were parsed',
    '导入完成': 'Import complete',
    '解析失败': 'Parse failed',
    '先选择一个目录': 'Select a catalog first',
    '先在右侧选择一个目录': 'Select a catalog on the right first',
    '先在左侧选择一个样式': 'Select a style on the left first',
    '样式名已存在': 'A style with this name already exists',
    '新样式名称': 'New style name',
    '名称无效或已存在': 'Invalid or duplicate name',
    'Default 样式不可删除': 'The Default style cannot be deleted',
